...
IS
	...
	BEGIN
		...
		IF ((SELECT EXTRACT(HOUR FROM DEBUT) d FROM PROGRAMMATION) < 12)
			MERGE INTO DIM_PERIODE T1 USING (SELECT 'MATIN' FROM DUAL) ON (T1.PERIODE = T2.d)
			WHEN NOT MATCHED THEN INSERT (T1.PERIODE) VALUES ('MATIN');
		ELSE IF ((SELECT EXTRACT(HOUR FROM DEBUT) d FROM PROGRAMMATION) > 18)
			MERGE INTO DIM_PERIODE T1 USING (SELECT 'SOIR' FROM DUAL) ON (T1.PERIODE = T2.d)
			WHEN NOT MATCHED THEN INSERT (T1.PERIODE) VALUES ('SOIR');
		ELSE
			MERGE INTO DIM_PERIODE T1 USING (SELECT 'APRES-MIDI' FROM DUAL) ON (T1.PERIODE = T2.d)
			WHEN NOT MATCHED THEN INSERT (T1.PERIODE) VALUES ('APRES-MIDI');
		END IF;

		MERGE INTO DIM_JOUR T1 USING (SELECT EXTRACT(DAY FROM DEBUT) d FROM PROGRAMMATION) T2 ON (T1.JOUR = T2.d)
		WHEN NOT MATCHED THEN INSERT (T1.JOUR) VALUES (T2.d);

		MERGE INTO DIM_SEMAINE T1 USING (SELECT TO_CHAR(DEBUT,'IW') d FROM PROGRAMMATION) T2 ON (T1.SEMAINE = T2.d)
		WHEN NOT MATCHED THEN INSERT (T1.SEMAINE) VALUES (T2.d);

		MERGE INTO DIM_MOIS T1 USING (SELECT (TO_DATE('01/'||EXTRACT(MONTH FROM DEBUT)||'/'||EXTRACT(YEAR FROM DEBUT), 'dd/mm/yyyy') d FROM PROGRAMMATION) T2 ON (T1.MOIS = T2.d)
		WHEN NOT MATCHED THEN INSERT (T1.MOIS) VALUES (T2.d);

		MERGE INTO DIM_ANNEE T1 USING (SELECT (TO_DATE('01/01'||EXTRACT(YEAR FROM DEBUT), 'dd/mm/yyyy') d FROM PROGRAMMATION) T2 ON (T1.ANNEE = T2.d)
