...
BEGIN
  ...
  i:=1;
  SELECT ACTORS BULK COLLECT INTO chaineActeur FROM movies_ext;
  --SELECT regexp_substr(ACTORS , '^\[\[(.*)\]\]$', 1, 1, '', 1) BULK COLLECT INTO chaineActeur FROM movies_ext;

  logevent('clob', 'apres le select');

  FOR cpt IN chaineActeur.FIRST..chaineActeur.LAST LOOP
    IF(LENGTH(chaineActeur(cpt)) > 0) THEN
      LOOP
        tmp := regexp_substr(chaineActeur(cpt) , '^\[\[(.*)\]\]$', 1, 1, '', 1);
  ...
  logevent('clob', 'fin du traitement');
