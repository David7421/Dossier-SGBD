...
BEGIN
	...
	--AFFICHES
	INSERT INTO AFFICHE@CBB.DBL(ID, IMAGE) SELECT AFFFICHE.ID, IMAGE FROM AFFICHE INNER JOIN FILM ON FILM.AFFICHE = AFFICHE.ID WHERE TOKEN IS NULL;
	...
	--COPIE FILM
	INSERT INTO FILM_COPIE@CBB.DBL (FILM_ID, NUM_COPIE) SELECT FILM_ID, NUM_COPIE FROM FILM INNER JOIN FILM_COPIE ON ID = FILM_ID WHERE TOKEN IS NULL;
