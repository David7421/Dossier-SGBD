...
EXCEPTION
	WHEN OTHERS THEN
		IF SQLCODE = -2291 THEN :NEW.TOKEN := 'KO'; LOGEVENT('CB : TRIGGER COPIECOTESAVIS', 'Copie ratée (Foreign Key) => ' || SQLCODE || ' : ' || SQLERRM);
		ELSE LOGEVENT('CB : TRIGGER COPIECOTESAVIS', 'Copie ratée => ' || SQLCODE || ' : ' || SQLERRM); RAISE;
		END IF;
END;
/
